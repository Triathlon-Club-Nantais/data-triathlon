...
- [X] T002 Écrire les tests du modèle `ParticipationTeammate` dans `backend/tests/test_repositories/test_participation_repository.py` : PK composite (un athlète une fois par résultat), cascade à la suppression de la participation, refus de suppression d'un athlète encore lié (non testable : SQLite n'applique pas les clés étrangères, contrôle en PostgreSQL seulement), relation `Participation.teammates` ordonnée par insertion
