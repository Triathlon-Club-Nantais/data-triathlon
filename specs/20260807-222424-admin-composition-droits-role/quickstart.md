...
- [ ] Les trois rôles livrés sont listés, chacun avec son nom, sa description et
      son nombre de porteurs.
- [ ] Déplier « Modérateur » : « Consulter les signalements » et « Instruire les
...
- [ ] Les sept fonctionnalités apparaissent dans l'ordre du serveur : « Rôles et
      accès », « Groupes d'appartenance », « Chronométreurs signalés »,
      « Qualité des données », « Épreuves », « Coureurs », « Résultats ».
      Comparer avec `curl` :

      ```bash
      curl -s -b cookies.txt localhost:<port>/api/v1/admin/permissions | jq -r '.[].feature'
      ```

- [ ] Déplier « Administrateur » : le panneau **annonce le statut** — franchit
...
- [ ] Déplier « Validateur » (rôle **livré**) : le renommer en « Validateur
...
- [ ] Lui cocher « Corriger une épreuve », enregistrer, recharger : le pouvoir
      est là.
- [ ] Le décocher, enregistrer : il n'y est plus.
...
- [ ] Recharger `/admin/droits` : `legacy:oldpower` apparaît dans un bloc
      **distinct** des cases de l'inventaire, annoncé comme périmé et sans effet.
- [ ] **Renommer seulement** le rôle et enregistrer → recharger : le code périmé
      est **toujours là**. C'est l'invariant de `research.md` §D6 (`PATCH`
      n'envoie que ce qui a changé).
- [ ] Modifier la composition et enregistrer : l'écran a prévenu que le code
...
- [ ] « Créer un rôle » : nom « Bénévole ». L'identifiant se propose tout seul
      (`benevole`) et reste corrigeable.
- [ ] Cocher un pouvoir, valider : le rôle apparaît avec **0 porteur**.
- [ ] Recréer un rôle avec le même identifiant : le refus du serveur s'affiche,
      la saisie est conservée.
- [ ] Supprimer « Bénévole » (confirmation) : il disparaît.
- [ ] Sur « Validateur » et « Administrateur » (livrés) : le bouton de
...
- [ ] Recharger : la suppression de ce rôle est désactivée et annonce **le
      nombre de porteurs**.
...
- [ ] `/admin/droits` s'ouvre et liste les rôles.
...
Deux fenêtres, deux comptes habilités à composer.

- [ ] Fenêtre A : ouvrir le panneau d'un rôle, cocher un pouvoir, **ne pas**
      enregistrer.
- [ ] Fenêtre B : ajouter un autre pouvoir au **même** rôle, enregistrer.
- [ ] Fenêtre A : provoquer un rafraîchissement (revenir sur l'onglet suffit ;
      créer un rôle depuis la même page aussi) → un encadré annonce que le rôle a
      été modifié ailleurs, et « Enregistrer » est désactivé.
- [ ] « Repartir de la version à jour » reprend le panneau sur l'état du serveur,
      pouvoir de B compris.

## 8. Refus de lecture (FR-017, FR-018)

- [ ] Se connecter avec un compte portant **un seul** pouvoir hors du domaine
      des rôles (par exemple `quality:override`), puis atteindre `/admin/droits`
      directement par l'URL — la navigation ne l'y mène pas.
- [ ] L'écran affiche « Accès refusé », **pas** une liste vide et **pas**
      « aucun rôle ».
- [ ] Supprimer le cookie de session et recharger : le message parle de session
...
- [ ] Tenter de retirer le statut de superutilisateur au rôle « Administrateur »
