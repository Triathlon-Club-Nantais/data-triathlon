...
- [ ] T001 [P] [US1] Test repository : `list_validated_for_athlete()` ne rend que les lignes `"validee"` de l'athlète donné, triées `created_at desc`, aucune autre saison/athlète mélangée dans `backend/tests/test_repositories/test_volunteer_action_repository.py`
- [ ] T002 [P] [US1] Test service : `list_validated_for_athlete()` délègue au repository dans `backend/tests/test_services/test_volunteer_action_service.py`
- [ ] T003 [P] [US1] Test API : `GET /admin/athletes/{id}/volunteer-actions/validated` — `200` liste filtrée (title/description `null` toléré), `403` sans `athletes:volunteer_validate`, liste vide sur athlète sans action validée dans `backend/tests/test_api/test_admin_volunteer_actions_api.py`
...
- [ ] T005 [US1] Implémenter `list_validated_for_athlete()` dans `backend/app/repositories/volunteer_action_repository.py` — fait passer T001
- [ ] T006 [US1] Implémenter `volunteer_action_service.list_validated_for_athlete()` dans `backend/app/services/volunteer_action_service.py` — fait passer T002 (depends on T005)
- [ ] T007 [US1] Ajouter `GET /admin/athletes/{athlete_id}/volunteer-actions/validated` au router `backend/app/api/v1/admin_volunteer_actions.py`, gardé par `require_permission(P.ATHLETES_VOLUNTEER_VALIDATE)` — fait passer T003 (depends on T006)
- [ ] T008 [P] [US1] Ajouter le type `AdminVolunteerActionOut` (title/description `string | null`) dans `frontend/lib/types.ts`
- [ ] T009 [P] [US1] Ajouter `listValidatedVolunteerActions(athleteId)` dans `frontend/lib/api/client.ts` (depends on T008 pour le type de retour)
