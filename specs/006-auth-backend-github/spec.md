...
- **FR-001**: Le système DOIT exposer un endpoint qui redirige l'utilisateur vers l'écran d'autorisation GitHub, en incluant un paramètre `state` cryptographiquement aléatoire servant de protection CSRF. Le `state` est signé et horodaté ; sa fenêtre de vie est de 10 minutes. Il n'est **pas** à usage unique côté TCN — GitHub refusant un `code` déjà consommé, un rejeu du callback tombe de toute façon sur une erreur d'échange (FR-004).
...
- **FR-003**: Le système DOIT rejeter tout callback dont le `state` est absent, inconnu, ou expiré, sans ouvrir de session. Un `state` déjà consommé retombe sur FR-004 via GitHub.
...
- **FR-022**: Les **requêtes SQL** sur la table `users` DOIVENT être formulées dans le repository (`user_repository.py`) — aucune requête SQL (`db.query`, `db.execute`, etc.) n'est autorisée dans le router ni dans le service. La gestion de transaction (`db.commit()`, `db.rollback()`) reste autorisée dans le router, conformément à la convention maison (`admin.py`, `participations.py`). Reformulation issue de la review B PR #159 pour aligner la règle sur le code existant.
...
- **URL de retour du callback** : une seule URL de callback, configurée statiquement, pointant vers le **domaine du frontend Vercel** — pas celui du backend Render. En prod, le front proxifie `/api/*` vers le back via un rewrite ; les cookies `tcn_oauth_state` et `tcn_session` sont attribués par le navigateur au domaine de l'origine appelée (Vercel), donc GitHub doit rappeler ce même domaine. Un callback pointant sur Render court-circuiterait Vercel au retour d'autorisation, le cookie `tcn_oauth_state` (host-only sur Vercel) ne serait pas envoyé, et le callback échouerait en 400 « État CSRF invalide. » à chaque connexion. La destination finale après retour (par exemple `/admin` en local, `/` en production tant qu'aucun back-office n'est livré) est portée par une variable de configuration, pas par un paramètre d'entrée du callback — un paramètre d'entrée serait une porte ouverte à l'« open redirect ».
