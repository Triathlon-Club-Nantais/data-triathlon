...
- [X] T001 Écrire le script d'extraction **jetable** (hors dépôt : dans le scratchpad de session, pas dans `scripts/`) qui télécharge les pages du panel et n'en garde que `h2.date`, `h2.name`, le `select.select_epreuve` entier, et pour chaque `div.results_epreuve` la ligne `div.table-row.head` suivie des seules lignes `a.table-row.body` sélectionnées ci-dessous, attributs `data-*` conservés — prérequis de T002 à T008
- [X] T002 [P] Créer `tests/fixtures/chronoweb/event_triathlon.html` depuis Oléron 2024 (`event=323`) : les 3 épreuves du `select` (dont `1147` « Triathlon M » et `1148`), et sur l'épreuve 1147 les lignes des dossards 360 (3 passages : points 1/8/14, cumuls 00:24:24 / 01:31:34 / 02:13:26) et 347 (3 passages, transitions attendues 00:11:03 et 00:03:38), plus un **non-finisher** (passages aux points intermédiaires, aucun au point final) et un **finisher à point vélo manquant**. Garder aussi une ligne de catégorie `MASC`, `FEM` ou `MIXT` en épreuve individuelle (p. ex. dossard 422). Si `event=323` ne contient aucun finisher à point manquant, extraire ce cas depuis Oléron 2025 / épreuve 1291 (450 passages Natation, 439 Vélo, 445 Course) dans une fixture dédiée `event_point_manquant.html` et ajuster les tests T023 en conséquence — on ne fabrique pas un cas absent de la source
- [X] T003 [P] Créer `tests/fixtures/chronoweb/event_duathlon.html` depuis Toulouse 2024 (`event=296`) : une épreuve au motif `Course → Vélo → Course` (2 dossards, 3 passages chacun), l'épreuve « S Relais » (2 lignes d'équipe dont `TRIPOTES TEAM GOLFECH RELAIS1`, catégorie `MIXT`) et un classement dérivé mono-point (« Challenge 1er Tour », point `Vélo` seul)
- [X] T004 [P] Créer `tests/fixtures/chronoweb/event_aquathlon_relais.html` depuis La Verrerie 2025 (`event=334`) : « Aquathlon Team Relais », **8 points** alternant `Natation` et `Course`, deux équipes aux 8 passages complets (dont `CREUSOTRI`, catégorie `MASC`)
- [X] T005 [P] Créer `tests/fixtures/chronoweb/event_mono_point.html` depuis ALEFPA Trail 2025 (`event=356`) : au moins deux épreuves (« 53 km », « 13 km ») à **un seul** point `Course`, 2 lignes chacune — aucune transition n'y est déductible
- [X] T006 [P] Créer `tests/fixtures/chronoweb/event_sans_classement.html` depuis Chalain 2015 (`event=146`) : `h2.name` et `select.select_epreuve` présents, **aucune** ligne de passage
- [X] T007 [P] Créer `tests/fixtures/chronoweb/event_inconnu.html` depuis `resultats_evenement.php?event=99999` : réponse 200 portant « Aucun évènement trouvé avec cet ID », **sans** `h2.name`
- [X] T008 [P] Créer `tests/fixtures/chronoweb/catalogue.html` : extrait de `/resultats.php` limité à trois lignes — celle portant `resultats_evenement.php?event=323` (`div.table-cell.location` = « St Georges d'Oléron »), celle de `event=296`, et une ligne **sans** cellule `location` (cas de la ville absente)
...
- [X] T009 Écrire `tests/test_chronoweb.py` : docstring de module en anglais (principe I) datant et situant les fixtures, helper `_fixture(name)`, `FakeResponse`, et un `FakeClient` qui **route par URL** (`resultats_evenement.php` → fixture d'événement, `resultats.php` → catalogue) et **compte ses appels** — la garantie de cardinalité (FR-020) se vérifie sur ce compteur. Y adosser un premier test rouge de `_parse_event_meta` sur `event_triathlon.html` : nom « Triathlon d'Oléron 2024 », `event_date == date(2024, 10, 6)`, `city == ""` à ce stade
- [X] T010 Créer `app/scrapers/chronoweb.py` : docstring de module (anglais), `PROVIDER_NAME`, `BASE_URL`, `HEADERS`, les dataclasses internes `EventMeta` / `RaceMeta` / `Passage` / `Runner` de [data-model.md](./data-model.md), et `_parse_event_meta` (`h2.name`, `h2.date` en `jj/mm/aaaa`) — rendre T009 vert
- [X] T011 Test rouge de `_parse_races` dans `tests/test_chronoweb.py` : sur `event_triathlon.html`, 3 `RaceMeta` lues sur `select.select_epreuve > option[value]` (`race_id`, `label`), `event_type` résolu par `classify.classify_event_type(label, contexte=EventMeta.name)`, `is_relay` faux ; sur `event_duathlon.html`, « S Relais » ressort `is_relay` vrai ; sur `event_sans_classement.html`, les épreuves du `select` sans `div.results_epreuve` associé n'entraînent aucune exception
- [X] T012 Implémenter `_parse_races` dans `app/scrapers/chronoweb.py` (dont la détection de relais par libellé — `relais` / `duo` / `team`, sans accent ni casse, research R6) — rendre T011 vert
- [X] T013 Test rouge de détection dans `tests/test_registry.py`, en **étendant les listes paramétrées existantes** plutôt qu'en écrivant des assertions parallèles (principe VI) : ajouter `chronoweb.com` à `_JETONS_PROVIDERS` — les 6 gabarits de contournement de #49 (jeton en query, en path, en fragment, host sosie `evil-chronoweb.com`, sous-domaine hostile, userinfo `chronoweb.com@169.254.169.254`) doivent tous tomber sur `playwright` —, et deux lignes à `_ROUTAGE_LEGITIME` : `https://chronoweb.com/resultats_evenement.php?event=323` et la forme `www.`. Y adosser un test explicite de `is_supported` sur ces deux URLs. **Attendu à ne pas confondre** : `https://timepulse.fr@chronoweb.com/x` a pour host **réel** `chronoweb.com` (`registry._url_host` lit `hostname`, pas `netloc`) — il est donc légitimement détecté `chronoweb`, contrairement à `evil-chronoweb.com` ; et `https://[oops/x` reste `playwright` (host illisible → `""`). Les 12 providers existants restent détectés à l'identique (non-régression, SC-006)
- [X] T014 Ajouter `ChronoWebProvider(HostMatchedProvider)` (`name = "chronoweb"`, `_HOSTS = ("chronoweb.com",)`, délégation à `chronoweb.scrape_event_all`, **aucune** surcharge de `matches`) et son entrée dans `PROVIDERS` de `app/scrapers/registry.py`, avant `T2AreaProvider` — rendre T013 vert
...
- [X] T035 [US2] Test rouge de `canonical_url` sur les formes du Sheet : `resultats_participant.php?event=347&epreuve=1234&bib=599` et `resultats_participant.php?event=347&epreuve=1235&bib=1563` rendent la **même** URL canonique `resultats_evenement.php?event=347` (FR-016) ; les 4 graphies d'Oléron 2024 du Sheet se réduisent à une seule URL canonique
- [X] T036 [US2] Test rouge de bout en bout : `scrape_event_all(<url de fiche individuelle>)` interroge la page d'**événement** et jamais `resultats_participant.php` (assertion sur les URLs vues par le `FakeClient`), et rend l'événement entier ; deux invocations sur deux graphies du même événement rendent des listes identiques champ à champ (FR-002, US2 §3)
- [X] T037 [US2] Étendre `canonical_url` à `resultats_participant.php` (troncature vers son événement) — le corps de `scrape_event_all` ne change pas, il consomme déjà l'URL canonique — rendre T035 et T036 verts
...
- [X] T038 [US3] Test rouge du refus d'URL : `files/pdf/Resultats_Triathlon_dOlron_2025.zip` et `resultats_evenement.php` sans paramètre → `ValueError` dont le message **en français** nomme la forme attendue `resultats_evenement.php?event=<id>` ; **aucun appel HTTP émis** (compteur du `FakeClient` à 0) — le scraper ne doit jamais tenter de parser un binaire (FR-017)
- [X] T039 [US3] Test rouge de la distinction introuvable / vide : `event_inconnu.html` (aucun `h2.name`) → `ValueError` « événement introuvable » ; `event_sans_classement.html` (`h2.name` présent, zéro ligne) → liste vide **sans** exception (FR-018)
- [X] T040 [US3] Implémenter les deux gardes dans `app/scrapers/chronoweb.py` : absence de paramètre `event` rejetée **avant** tout appel réseau, absence de `h2.name` rejetée après le premier appel — rendre T038 et T039 verts
...
- [X] T041 [P] Couvrir chronoweb dans `tests/test_integration_scrapers.py`, puis vérifier `uv run pytest -m integration -k chronoweb`. **Deux emplacements distincts**, et non deux entrées de `LIVE_URLS` : ce dictionnaire est **clé = nom de provider** et `test_detection` y asserte `detect_provider(url) == provider`, donc une seconde entrée chronoweb écraserait la première (clé identique) ou casserait l'assertion (clé inventée) — dans les deux cas le contrôle de SC-004 disparaîtrait sans bruit.
  - [X] (a) **une** entrée `"chronoweb"` dans `LIVE_URLS` : Oléron 2024, qui hérite ainsi des tests génériques du fichier (détection, participants exploitables, cohérence des statuts) ; y vérifier les effectifs publiés, 3 épreuves / 854 participants (SC-002)
  - [X] (b) un test dédié **hors du dictionnaire**, sur le patron de `test_t2area_epreuve_complete` / `test_runnerbreizh_importe_toute_lepreuve_depuis_une_page_intermediaire` : **Triathlon de Dijon 2026**, 8 épreuves / 1 622 participants sur une page de 4,5 Mo — seul contrôle de SC-004 (aucun participant dupliqué ni omis sur le plus gros événement du panel) et de la charge relevée au cadrage. Assertion de non-duplication `(event_name, bib_number)` sur l'ensemble des 8 épreuves, et durée observée journalisée pour la confronter aux ~3 s mesurées en local (research R1) — le seul environnement où la tenue en mémoire ait été vérifiée
- [X] T042 [P] Ajouter `chronoweb: "ChronoWeb"` à `PROVIDER_LABELS` dans `frontend/lib/constants.ts` et vérifier `npm test` — le front ne liste jamais les providers, cette entrée n'est qu'un libellé commercial (son absence ne vaut pas « non supporté »)
- [X] T043 [P] Documenter le fournisseur dans `AGENTS.md`, section « Fournisseurs supportés » : une ligne = un **passage** et non un participant, événement entier en une requête sans JS, temps total et rangs du **seul point final** (aucun rang intermédiaire promu), transitions **calculées** et non publiées, motif de points reconnu → slots / sinon libellés de la source, plafond de **2 requêtes** par import, absence de club (donc hors `scope=club`), de date de naissance et de distinction DNS/DSQ, canonicalisation par allowlist du seul `event`, refus de l'archive ZIP, distinction événement introuvable / sans classement, limites du classifieur partagé laissées hors périmètre, et renvoi au sondage
- [X] T044 Lancer `uv run ruff check .` depuis `backend/` et corriger les écarts du nouveau module et des fichiers touchés
- [X] T045 Lancer `uv run pytest -m "not integration"` depuis `backend/` : suite complète verte, aucune régression sur les 12 autres fournisseurs (SC-006)
- [X] T046 Exécuter le [quickstart](./quickstart.md) §2 à §6 : détection, scrape à sec (854 participations, 3 `event_name`, dossard 360 vérifié champ à champ, `city`), import réel puis réimport **idempotent** (854 ajoutés puis 0 ajouté / 854 déjà en base), et les 5 formes d'URL du tableau §6 à la main
- [X] T047 Vérifier le bout en bout **côté API** : `GET /scrape/detect` → `{provider: "chronoweb", supported: true}`, `POST /scrape/event/stream` → phases SSE jusqu'à `done` sur Oléron 2024, `POST /scrape/event` sur l'URL d'archive ZIP → **422** portant le message français verbatim de T038
...
Cinq écarts entre la liste de tâches et ce que la source a montré. Tous tranchés
en **re-sondant**, jamais en ajustant un test au code.

| # | Tâche | Ce qui était prévu | Ce qui a été mesuré, et fait |
| --- | --- | --- | --- |
| 1 | T002 / T023 | un finisher à point vélo manquant dans `event=323` | **Absent** : les 344 dossards de l'épreuve 1147 se répartissent en (1,8,14) ×338, (1,) ×4, (1,8) ×2. Le cas a donc été extrait d'Oléron 2025 (`event=358`, épreuve 1291, dossard 435) dans la fixture prévue en repli, `event_point_manquant.html` — **8 fixtures** au lieu de 7. |
| 2 | T008 | une ligne de catalogue **sans** cellule `location` | **Aucune** des 222 lignes n'en manque : le cas n'existe pas à la source, il n'a pas été fabriqué. Le chemin « sans ville » reste couvert par les deux cas réels — catalogue en échec HTTP, et événement absent du catalogue. Au passage, le lien du catalogue porte `event` **et** `epreuve` : la ligne se retrouve en comparant le **paramètre**, jamais la sous-chaîne `?event=323`. |
| 3 | T027 / SC-007 | 15 segments sur l'aquathlon relais (8 étapes + 7 temps morts) | **8**. Les 7 écarts `cumul − intervalle − cumul précédent` valent **zéro** sur les 14 équipes de l'épreuve (points contigus), et un temps mort nul n'est pas enregistré (FR-008). SC-007, la clarification du 2026-07-30 et research R3 ont été corrigés ; la règle d'intercalage, elle, reste couverte par un cas construit. |
| 4 | T031 | dossard 422 « découpé normalement en nom/prénom » | `split_athlete_name("JEAN BONNEAU")` rend `("JEAN BONNEAU", "")` — limite **documentée** du helper partagé sur un nom tout en majuscules, pas le chemin relais. Le test l'assert et le dit. Rien à corriger dans le scraper : la contre-épreuve visée (`is_relay` faux, genre `M`) tient. |
| 5 | T017 | « un temps illisible laisse le segment vide » | `utils.normalize_time` rend la chaîne **telle quelle** quand elle ne la reconnaît pas, et aucun temps illisible n'existe au panel. Aucune règle locale n'a été ajoutée (principe VI) ; seule la garde « ne lève pas » est vérifiée. |

Trois tâches se sont révélées **sans travail** : T026 et T028 (les 5 motifs et le
chemin générique étaient déjà posés par T024, qui les tient de la même table
mesurée R2) et T037 (`canonical_url`, reconstruite par allowlist du seul `event`
dès T016, traitait déjà `resultats_participant.php`). Leurs tests, eux, ont été
écrits et sont verts : ce sont des vérifications, pas des implémentations.

**Vérifications finales** — `ruff check .` propre ; `pytest -m "not integration"`
1 753 tests verts (71 nouveaux sur `test_chronoweb.py`, 176 sur `test_registry.py`) ;
`pytest -m integration -k chronoweb` 5 tests verts sur le site réel (Oléron 2024 :
854 participants / 3 épreuves ; Dijon 2026 : 1 622 / 8, sans doublon) ;
`npm test` 258 tests verts ; quickstart §2 à §6 conforme, y compris le réimport
idempotent (854 ajoutés puis 0 / 854 déjà en base) et les 5 formes d'URL ; API :
`detect` → `{chronoweb, true}`, SSE `scraping → saving → done` (185 importés sur
la Verrerie 2025), archive ZIP → **422** portant le message français verbatim.
