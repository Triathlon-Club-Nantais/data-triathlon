...
- [ ] T057 Préparer la fin de branche : `requesting-code-review`, puis le sous-agent `ui-ux-review` (la branche touche `frontend/`) — en lui signalant explicitement que `tcn/Modal` n'a **ni piège à focus ni restauration du focus**, limite des trois modales publiques existantes que cette feature n'aggrave ni ne corrige (D8) —, puis `verification-before-completion` et `finishing-a-development-branch`.
