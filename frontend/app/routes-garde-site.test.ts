 ...
 * mauvais côté de `app/(protege)/` — une erreur invisible à la lecture d'un
 ...
 * `/admin/acces`, et il passe par `/login`. Rangé sous `(protege)`, ce chemin se
 * referme sur lui-même : `/admin` → `/login` → `/acces` → impasse.
 */
const APP = join(__dirname);
const GROUPE_PROTEGE = join(APP, "(protege)");
...
describe("Placement des routes vis-à-vis de la garde d'accès au site (#509)", () => {
  ...
  const protegees = dossiers(GROUPE_PROTEGE);
  ...
  it.each(["login", "acces", "benevoles", "admin"])(
    ...
    (route) => {
      ...
      expect(protegees).not.toContain(route);
    },
  );
  ...
  it.each(["dashboard", "resultats", "athletes", "courses", "club", "carte", "ajouter"])(
    ...
    (route) => {
      expect(protegees).toContain(route);
    },
  );
});
