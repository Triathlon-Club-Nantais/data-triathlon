 ...
 * elle se rend donc quelle que soit la valeur de `session` (`null` compris),
 * dès que sa résolution est connue. Les deux sections restent néanmoins
 * derrière `isPending` : les afficher plus tôt ferait apparaître le
 * formulaire de crédit avant l'auto-déclaration pour un visiteur déjà
 * connecté, puis le repousser dès la résolution — un décalage de mise en
 * page (`frontend/AGENTS.md`, « Les trois surfaces qui propagent l'athlète
 * retenu », #503) que `isPending` évite en ne rendant rien tant que l'état
 * n'est pas connu, résolu quasi instantanément pour un visiteur sans signal
 * de connexion (`useSession`, aucun appel réseau déclenché).
 */
