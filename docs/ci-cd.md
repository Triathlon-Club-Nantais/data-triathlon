...
> procédure (#169) : `/admin/sessions` depuis le back-office, ou
> `uv run python -m app.cli revoke-sessions --all` sur le serveur — ce second
> chemin restant praticable le jour où c'est du back-office qu'on se méfie.
> Pour une seule personne : `revoke-sessions --email <adresse>`. Ne plus passer
> par `psql` : c'est ce que #169 a retiré.
>
