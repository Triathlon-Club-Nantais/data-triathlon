...
| --- | --- | --- |
| Klikego, Breizh Chrono | même back-office : Breizh Chrono **réutilise** `klikego._parse_detail` — factoriser dans `klikego.py`, ne jamais dupliquer. Deux façades (`resultats.`, `live.`), et splits fins sur la page détail, requêtée pour **tous** les participants chez Klikego (phase C parallélisée, #583) ; Breizh Chrono, lui, n'a pas cette parallélisation et continue de réserver sa page détail aux seuls membres du TCN. La date d'un heat vient de `live5/index.jsp` (`klikego_platform.parse_live_index`), servie aussi par `www.klikego.com` : le fan-out Klikego y lit la date **par heat** (#972), la date d'événement n'étant qu'un repli (page événement et `--single-heat`). Seule la date lue par heat (`ScrapedResult.heat_dated`) redate à l'import une épreuve déjà rapprochée par la règle R (`import_service._redate_heats`). | `docs/scrapers/breizhchrono.md` (Breizh Chrono) |
| TimePulse, Sportinnovation | TimePulse écrit l'abandon et la disqualification **à la place du temps**, dans `t` du `<R>` (`Abandon`, `Disqualifié`, épreuve 3232, #899) : DNF/DSQ, sans temps, sans rang, et sans les splits partiels (choix assumé, même chemin que le flag `np`). Le parcours se classe avec le nom d'événement en appoint (`contexte=`, #973) : « Course nature » d'un « Maré Trail » est un trail. | — |
