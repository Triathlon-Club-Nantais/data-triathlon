...
def _ranks_by_bib(db_session) -> dict[str, int | None]:
    course = course_repository.get_by_identity(db_session, NOM, JOUR, TYPE, False)
    assert course is not None
    return {
        p.bib_number: p.rank_overall
...
def test_finisher_with_empty_status_and_time_is_renumbered(db_session, patch_scraper):
    """#940: most scrapers leave `status=""` on finishers (derived only at
    persistence). A total time is enough to enter the renumbered batch."""
    patch_scraper(
        [
            _result("1", 1, "1:00:00", status=""),
            _result("2", 1, "1:05:00", status=""),
            _result("3", 2, "1:10:00", status=""),
            _result("4", 2, "1:02:00", status=""),
        ]
    )
    import_service.import_event(db_session, URL, _settings())

    assert _ranks_by_bib(db_session) == {"1": 1, "2": 3, "3": 4, "4": 2}


def test_empty_status_without_time_derives_dnf_and_is_excluded(db_session, patch_scraper):
    """#940: without a time, an empty status derives to DNF and stays out of the
    renumbered batch (included, its unreadable time would push it to rank 3)."""
    patch_scraper(
        [
            _result("1", 1, "09:00:00", status=""),
            _result("2", 1, "10:00:00", status=""),
            _result("3", 2, "", status=""),
        ]
    )
    import_service.import_event(db_session, URL, _settings())

    assert _ranks_by_bib(db_session) == {"1": 1, "2": 2, "3": 2}
