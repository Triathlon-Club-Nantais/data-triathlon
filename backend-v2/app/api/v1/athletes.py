...
from app.repositories import athlete_repo, participation_repo
...
def list_athletes(
    ...
    return athlete_repo.search(db, name=name, club=club, page=page, page_size=page_size)


@router.get("/athletes/{athlete_id}")
def get_athlete(athlete_id: int, db: Session = Depends(get_db)):
    athlete = athlete_repo.get(db, athlete_id)
    if not athlete:
        raise NotFoundError("Athlète introuvable")
    participations = participation_repo.list_for_athlete(db, athlete_id)
    return {
